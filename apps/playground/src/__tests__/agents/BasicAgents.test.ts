...
describe('Basic Agents', () => {
  ...
  describe('WriterAgent', () => {
    ...
    it('should have correct properties', () => {
      ...
      expect(writerAgent.capabilities).toEqual([
        {
          name: 'write',
          description: 'Can produce written summaries or narrative content',
        },
      ]);
    });
    ...
    it('should extract topic and generate summary', async () => {
      ...
      expect(response).toEqual({
        ...
        to: 'user',
        content:
          'The "Fitness tracking app" project aims to help users improve their wellbeing by tracking relevant metrics, setting achievable goals, and receiving timely feedback. By focusing on simplicity and user experience, this app could empower individuals to build lasting habits through thoughtful design.',
        traceId: 'trace-123',
        conversationId: 'conv-123',
      });
    });
    ...
    it('should handle message without prefix', async () => {
      ...
      expect(response.content).toContain(
        'The "Just write something" project aims'
      );
    });
  });
  ...
  describe('DesignerAgent', () => {
    ...
    it('should have correct properties', () => {
      ...
      expect(designerAgent.capabilities).toEqual([
        {
          name: 'design_ui',
          description: 'Creates UI layout designs and visual concepts',
        },
      ]);
    });
    ...
    it('should return design response', async () => {
      ...
      expect(response).toEqual({
        ...
        to: 'user',
        content:
          'Designed a clean, responsive layout with a top navbar, hero section, and feature cards.',
        traceId: 'trace-123',
        conversationId: 'conv-123',
      });
    });
  ...
  describe('AnalystAgent', () => {
    ...
    it('should return analytical insights', async () => {
      ...
      expect(response).toEqual({
        ...
        to: 'user',
        content:
          'From an analytical perspective, success hinges on daily retention, clear feedback loops, and optional community integration. You should also define KPIs early.',
        traceId: 'trace-123',
        conversationId: 'conv-123',
      });
    });
  ...
  describe('SpiralResolverAgent', () => {
    ...
    it('should handle message without traceId', async () => {
      ...
      expect(response.content).toContain(
        'Loop detected in trace: unknown-trace'
      );
    });
  ...
  describe('EscalationManager', () => {
      ...
      expect(escalationManager.capabilities).toEqual([
        {
          ...
          description: 'Reports resolved spirals back to initiators',
        },
      ]);
    });
    ...
    it('should handle message without parentId', async () => {
      ...
      expect(response.content).toContain(
        'Resolution for trace trace-123 complete.'
      );
    });
    ...
    it('should handle message without traceId', async () => {
      ...
      expect(response.traceId).toBe('unknown-trace');
      expect(response.content).toContain(
        'Resolution for trace unknown-trace complete.'
      );
    });
  });
});
