...
describe('OrchestratorAgent', () => {
  ...
  describe('constructor', () => {
    it('should create orchestrator agent with correct properties', () => {
      ...
      expect(orchestratorAgent.capabilities).toEqual([
        {
          name: 'orchestrate',
          description: 'Coordinates tasks between agents and assembles results',
        },
      ]);
    });
  });
  ...
  describe('receiveMessage', () => {
    it('should orchestrate tasks and assemble results', async () => {
      ...
      const mockContext = (orchestratorAgent as unknown as { context: { findAgent: ReturnType<typeof vi.fn>; send: ReturnType<typeof vi.fn> } }).context;
      ...
      expect(response).toEqual({
        ...
        content: expect.stringContaining(
          'Here is your assembled result for: Create a fitness app'
        ),
        traceId: 'trace-123',
        conversationId: 'conv-123',
      });
    ...
    it('should handle agent failures with retry logic', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        '[WARN] [trace-123] designer failed, retrying once...'
      );
