...
export class PlannerAgent implements Agent {
  ...
  async receiveMessage(message: AgentMessage): Promise<AgentResponse> {
    ...
    console.log(
      `[MEMORY] [${traceId}] Planner has seen ${previousMemory.length} events so far.`
    );
    ...
    return new Promise((resolve) => {
      this.tracker.start(conversationId, idsToWaitFor, (responses) => {
        ...
        const research = responses[0] || {
          content: 'No research response received.',
        };
        const write = responses[1] || {
          content: 'No writer response received.',
        };
      ...
      if (researcherId) {
        this.context
          .send(researcherId, `Research this goal: ${goal}`, {
            traceId,
            conversationId,
      ...
      if (writerId) {
        this.context
          .send(writerId, `Summarize plan for: ${goal}`, {
            traceId,
            conversationId,
            parentId: message.from,
          })
